...
Интерактивный режим, можно пользоваться стрелками чтобы вызывать историю или редактировать, Enter вызовет последнюю набранную комманду:
```
...
***************
SIMULATOR ERROR: image was not loaded to simulator
***************

steps performed 0, current pc 0
(isa-sim): l
...
(isa-sim): d 0 40
dissasemble 40 words from address 0
      ...
      0x80	   0x82403	    LW	 T0, [ S0 + 0]
      0x84	  0x482483	    LW	 T1, [ S0 + 0x4]
      0x88	  0x882503	    LW	 T2, [ S0 + 0x8]
      0x8c	  0xc82583	    LW	 T3, [ S0 + 0xc]
      0x90	 0x1082603	    LW	 T4, [ S0 + 0x10]
      0x94	 0x1182023	    SW	 S0, [ S1 + 0]
      0x98	 0x1282223	    SW	 S0, [ S2 + 0x4]
      0x9c	 0x12888b3	   ADD	 S1,  S1, S2
(isa-sim): s 10
...
(isa-sim): st
             program running: no
       steps made from begin: 20
     steps made at last step: 10
              image was load: yes
                   was error: 0	
                 return code: 0
(isa-sim): trace off
...
tracing: off
(isa-sim): s
start 1 step from 0x28
steps performed 1, current pc 0x2c
(isa-sim): q
...
Добавлять новые команды просто, поэтому он будет прирастать. Пока доступные команды можно получить вызвав help (или h).
